...
def to_gluonts_univariate(hf_dataset: datasets.Dataset, subset_size=np.inf):
    series_fields = [
        col
    ...
    if subset_size == np.inf:
        # if we want to train on a smaller subset size, then we don't care about this assert statement
        assert len(gts_dataset) == dataset_length

    return gts_dataset


def load_and_split_dataset(backtest_config: dict, subset_size=np.inf):
    ...
    ds = datasets.load_dataset(
        hf_repo, dataset_name, split="train", trust_remote_code=trust_remote_code, cache_dir="data"
    )
    ds.set_format("numpy")

    # conditional logic for eval-ing on just a subset
    if subset_size < np.inf:
        subset_size = min(subset_size, len(ds))
        ds = ds.select(range(subset_size))

    gts_dataset = to_gluonts_univariate(ds, subset_size=subset_size)
