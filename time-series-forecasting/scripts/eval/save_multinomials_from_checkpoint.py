...
import sys
for pth in sys.path:
    if pth.endswith("/time-series-forecasting/scripts/eval"):
        sys.path.append(pth.replace("/time-series-forecasting/scripts/eval", "/time-series-forecasting"))
        break
...
def save_multinomials(trainer, step_number) -> None:
    """
    Evaluation loop, updated to work with Chronos
    """
    
    print("SAVING MULTINOMIALS FOR step_number = ", step_number)
    ...
    trainer.model.SAVE_CATEGORICAL_DISTRIBUTIONS = True
    MAX_NUM_TIME_SERIES_TO_EVALUATE = 64
    NUM_SAMPLES_PER_TIME_SERIES = 1

    SAVE_CATEGORICAL_DISTRIBUTIONS_PATH_BASE = os.path.join(trainer.args.output_dir, "multinomials")
    os.makedirs(SAVE_CATEGORICAL_DISTRIBUTIONS_PATH_BASE, exist_ok=True)

    for config in backtest_configs:
        dataset_name = config["name"]

        trainer.model.SAVE_CATEGORICAL_DISTRIBUTIONS_PATH = os.path.join(SAVE_CATEGORICAL_DISTRIBUTIONS_PATH_BASE, f"{dataset_name}.npy")
        prediction_length = config["prediction_length"]


        test_data = load_and_split_dataset(backtest_config=config)
        test_data_iter = iter(test_data.input)

        for _ in range(MAX_NUM_TIME_SERIES_TO_EVALUATE):

            try:
                next_time_series_in_dataset = [next(test_data_iter)] # just passes one element at a time
            except:
                print("SKIPPING!!")
                continue

            sample_forecasts = generate_sample_forecasts(
                next_time_series_in_dataset, 
                pipeline=trainer.pipeline,
                prediction_length=prediction_length,
                batch_size=1,
                num_samples=NUM_SAMPLES_PER_TIME_SERIES,
                temperature=temperature,
                top_k=top_k,
                top_p=top_p,
            )

    print(f"    Save multinomial loop finished. Total time: {(time.time() - start_time)/60} minutes")
...
def start_evaluation(
    ...
    if "in-domain" in eval_config_path:
        trainer.eval_dict_path = os.path.join(trainer.args.output_dir, "eval_dict-in-domain.json")
    elif "zero-shot" in eval_config_path:
    ...
    checkpoint_name = checkpoint_dirs[-1]
    ...
    save_multinomials(trainer, step_number)
