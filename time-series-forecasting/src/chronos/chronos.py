...
class MeanScaleMixedUniformBins(ChronosTokenizer):
    def __init__(
        self, low_limit: float, mid_limit_1: float, mid_limit_2: float, high_limit: float, sparse_bin_density: float, config: ChronosConfig
    ) -> None:
        self.config = config
        self.ll = low_limit
        self.ml1 = mid_limit_1
        self.ml2 = mid_limit_2
        self.hl = high_limit
        sparse_range = self.ml1 - self.ll + self.hl - self.ml2
        self.sparse_tokens1 = int(sparse_bin_density * self.config.n_tokens * (self.ml1 - self.ll)/sparse_range)
        self.sparse_tokens2 = int(sparse_bin_density * self.config.n_tokens * (self.hl - self.ml2)/sparse_range)
        self.dense_tokens = self.config.n_tokens - self.sparse_tokens1 - self.sparse_tokens2

        self.centers = torch.concat(
            (torch.linspace(
                self.ll,
                self.ml1,
                self.sparse_tokens1 - self.config.n_special_tokens - 1,
            ),
            torch.linspace(
                self.ml1,
                self.ml2,
    ...
    def _split_and_transform(
        ...
        attention_mask = ~torch.isnan(context)
        scaled_context = context / scale.unsqueeze(dim=-1)
        range2_mask = torch.logical_and(scaled_context >= self.ml1, scaled_context < self.ml2)
        range3_mask = scaled_context >= self.ml2

        num_tokens = self.config.n_tokens

        self.config.n_tokens = self.sparse_tokens1
        tokenizer = MeanScaleUniformBins(self.ll, self.ml1, self.config)
        token_ids, _, _ = tokenizer._input_transform(context, scale=scale)

        self.config.n_tokens = self.dense_tokens + self.config.n_special_tokens
        tokenizer = MeanScaleUniformBins(self.ml1, self.ml2, self.config)
        token_ids[range2_mask] = tokenizer._input_transform(context, scale=scale)[0][range2_mask] + self.sparse_tokens1 - self.config.n_special_tokens

        self.config.n_tokens = self.sparse_tokens2 + self.config.n_special_tokens
        tokenizer = MeanScaleUniformBins(self.ml2, self.hl, self.config)
        token_ids[range3_mask] = tokenizer._input_transform(context, scale=scale)[0][range3_mask] + (self.sparse_tokens1 + self.dense_tokens -
                                                                                                     self.config.n_special_tokens)

        token_ids[~attention_mask] = self.config.pad_token_id

        self.config.n_tokens = num_tokens
    ...
    def context_input_transform(
        ...
        token_ids, attention_mask = self._split_and_transform(context, scale)
    ...
    def label_input_transform(
        ...
        assert length == self.config.prediction_length
        token_ids, attention_mask = self._split_and_transform(context=label, scale=scale)
