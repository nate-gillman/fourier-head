...
# original location of this file in the tf codebase is:
# src/transformers/models/t5/modeling_t5.py
# see more: https://github.com/huggingface/transformers/blob/15585b81a525e3878321df4b13404c96690a2469/src/transformers/models/t5/modeling_t5.py#L1545
...
for path in sys.path:
    ...
    if path.endswith("/time-series-forecasting/scripts/train"):
        sys.path.append(path.replace("/time-series-forecasting/scripts/train", ""))
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            if hasattr(module, "lm_head") and not self.config.tie_word_embeddings:
                if isinstance(self.lm_head, Fourier_Head):
                    pass
                else:
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        LM_HEAD_TYPE = config.lm_head_type

        if LM_HEAD_TYPE == "linear":

            self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False) # maps torch.Size([32, 65, 384]) --> torch.Size([32, 65, 4096])

        elif LM_HEAD_TYPE == "fourier":

            dim_input               = config.d_model
            dim_output              = config.vocab_size
            num_frequencies         = config.fourier_kwargs["fourier_num_frequences"]
            regularizion_gamma      = config.fourier_kwargs["fourier_regularization_gamma"]
            const_inverse_softmax   = config.fourier_kwargs["fourier_inverse_softmax_constant"]
            init_denominator_weight = config.fourier_kwargs["fourier_weight_denominator"]
            init_denominator_bias   = config.fourier_kwargs["fourier_bias_denominator"]

            local_rank = int(os.environ.get('LOCAL_RANK',0))
            device = torch.device(f'cuda:{local_rank}')

            # maps torch.Size([bs, 65, 384]) --> torch.Size([bs, 65, 4096])
            self.lm_head = Fourier_Head(
                dim_input,
                dim_output,
                num_frequencies,
    ...
    def forward(
        ...
        lm_logits = self.lm_head(sequence_output) # torch.Size([32, 65, 384]) --> torch.Size([32, 65, 4096])

        if getattr(self, 'SAVE_CATEGORICAL_DISTRIBUTIONS', False):

            save_multinomials_path = self.SAVE_CATEGORICAL_DISTRIBUTIONS_PATH
            
            # save_multinomials_path = "output/07-07-mini-linear-8gpu-fixed-copy/multinomial_ETTh_linear.npy"   
            new_multinomials = torch.softmax(lm_logits[:,0], dim=-1) # (20, 4096)
            new_multinomials = new_multinomials.cpu().numpy() # (20, 4096)
            ...
            print(f"Saving array of shape {combined_array.shape} to {save_multinomials_path}")
