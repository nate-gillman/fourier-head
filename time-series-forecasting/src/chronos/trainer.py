...
class Trainer:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            # we added these, to save the losses
            single_gpu_training = torch.cuda.device_count() == 1
            primary_process_in_multi_gpu_training = False
            if torch.cuda.device_count() > 1:
                primary_process_in_multi_gpu_training = torch.distributed.get_rank() == 0
            this_is_the_main_process = single_gpu_training or primary_process_in_multi_gpu_training

            if this_is_the_main_process:
                train_metrics_path = os.path.join(self.args.output_dir, "train_dict.json")
                try:
                    with open(train_metrics_path, "r") as f:
                        train_metrics = json.load(f)
                except IOError:
                    print(f"initializing {train_metrics_path}")
                    train_metrics = {}
                train_metrics[str(self.state.global_step)] = logs
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        try:
            l_reg = (getattr(model.lm_head, 'l_reg', 0))
        except:
            l_reg = (getattr(model.module.lm_head, 'l_reg', 0)) # needed for DDP
        ...
        if labels is not None:
            ...
            if isinstance(outputs, dict) and "loss" not in outputs:
                # acceptable to return loss as the output in a dictionary
                raise ValueError(
            ...
            # isinstance(outputs, dict) when outputs is of type: transformers.modeling_outputs.Seq2SeqLMOutput
            # returns True!!!!
            # see docs: https://huggingface.co/docs/transformers/v4.44.1/en/main_classes/output#transformers.modeling_outputs.Seq2SeqLMOutput
            loss = outputs["loss"] if isinstance(outputs, dict) else outputs[0]

        # when gamma is 0, this is 0.
        loss = loss + l_reg
