...
if [[ "$dataset" == "beta" || "$dataset" == "gmm2" ]]; then
  for seed in 1 2 3 42; do
    ...
    echo "gmm head"
    echo "seed = $seed"
    python toy_synthetic_mle.py --head "gmm-mle" --n_gaussians 2 --dataset $dataset --seed $seed
  done
  ...
  for seed in 1 2 3 42; do
    ...
    echo "gmm head"
    echo "seed = $seed"
    python toy_synthetic_mle.py --head "gmm-mle" --n_gaussians 1 --dataset $dataset --seed $seed
  done
else
