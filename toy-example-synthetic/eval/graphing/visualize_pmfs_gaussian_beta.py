...
def plot_histogram_with_line(data, data_true, ax, title, label, label_true, ylim=0.20):
    ...
    ax.bar(bin_centers, data, width=0.04, color=color, alpha=0.7, label=f"{label} PMF; Smoothness = {smoothness:.4f}")
    if label_true:
...
def plot_combined_graphs(prefix, fourier, linear, gmm, true, idxs, output_fname):
    fourier = [np.load(prefix + fourier[j])[idxs[j]] for j in range(2)]
    linear = [np.load(prefix + linear[j])[idxs[j]] for j in range(2)]
