...
from eval.aggregate_mle import aggregate
...
def build_graphs(data, gmm_values, gmm_stds, output_fname, title=""):
        ...
        for gamma, (gamma_values, gamma_std) in values.items():
            ...
            if gamma == 'gamma1':
                ...
                axs[i].fill_between(generations,
                                ...
                                gmm_values[metric] + gmm_stds[metric],
                                color='tab:purple', alpha=0.2)  # Purple tunnel for the baseline
...
def main(output_dir):
    ...
    for dataset in ["gaussian", "gmm2", "beta"]:
        data, gmm_values, gmm_stds = aggregate(output_dir, [dataset], verbose=False)[0]
        ...
        build_graphs(
            ...
            gmm_values,
            gmm_stds,
            output_fname=os.path.join(output_dir, f"toy_mle_{dataset}_graph_varying_freqs.png"),
